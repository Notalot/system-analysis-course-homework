...
### Фитнес-функции на заявленные характеристики и ограничения

- **Availability/Fault tolerance** - нагрузочное тестирование
- **Security** - есть платные и бесплатные сервисы проверки уязвимостей, некоторые менеджеры пакетов (github, например) предлагают оценку уязвимостей установленных версий пакетов и даже могут при определенных условиях в автоматическом режиме эти зависимости обновлять
- **Maintainability** - в таск трекерах (вроде JIRA) есть возможность вычислить время жизни бага и составлять графики среднего показателя, из таких графиков можно узнать, например, что критичный баг не могут поправить (клиенты не могут создать заказ, не проходит оплата по техническим причинам нашего сервиса), скажем, 3 дня,
что очень плохой показатель, на это можно настроить нотификацию
- **Scalability** - ???
- **Modularity** - ???
- **Usability, Accessability** - существуют пакеты для тестирования Accessability, для Usability можно настроить обратную связь с клиентами, заказывать оценку сторонними сервисами, опрашивать тестировщиков
- **Релизный цикл для всей системы — месяц, для Найма работников — неделя** - посмотреть расписание релизов, держать связь с релизным менеджером
- **Reliability** - ???
- **соблюдение CatFinComplience** - может, во время разработки постоянно консультироваться с юристами, а потом периодически заказывать проверку
- **ожидаемое поведение системы: без сбоев и тупняков** - ??? у devops есть инструменты логирования работы сервисов, в том числе и проблем (Newrelic, Sentry), при пиковых нагрузках и неответе сервиса настроить нотификацию
